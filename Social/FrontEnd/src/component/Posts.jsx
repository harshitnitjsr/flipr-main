...
import {Like, unlike,comment,remove,edit} from '../api/api-post.js'
...
const editPost = ()=>{
  edit({postId :props.post._id},{ t: jwt.token}).then((data)=>{
    props.updatePosts(data);
  })
  window.location.reload()
}
...
const handleKeyDown = (event) => {
  ...
  <section className="posts border border_radius border-secondary mt-3  overflow-hidden p-0 white border position-relative">
        ...
        <Button  onClick={()=>{
                nav('/posts/edit/'+props.post._id)
              }}>EDIT</Button>
        </>)
        :null}
        {/* {props.post.userDetails.id==user1.id ? <i className="fa-regular fa-trash-can position-absolute pt-4 fs-5 trash" onClick={editPost}/>:null} */}
